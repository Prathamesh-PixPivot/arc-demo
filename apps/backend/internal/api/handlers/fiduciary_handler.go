...
func CreateNewFiduciaryHandler(fiduciaryService *services.FiduciaryService, licenseManager *licensing.LicenseManager) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// 1. Check License Limit (On-Prem)
		license, err := licenseManager.GetLicense()
		if err == nil && license.Type == licensing.LicenseTypeOnPrem {
			// Get current user count
			// We use ListFiduciaries with limit 1 just to get the Total count
			listResp, err := fiduciaryService.ListFiduciaries(repository.FiduciaryListParams{Limit: 1})
			if err != nil {
				log.Logger.Error().Err(err).Msg("failed to count fiduciaries for license check")
				writeError(w, http.StatusInternalServerError, "internal error")
				return
			}

			if err := licenseManager.CheckOnPremLimit("users", int(listResp.Total)); err != nil {
				log.Logger.Warn().Msg("License user limit reached")
				writeError(w, http.StatusPaymentRequired, "License user limit reached. Please upgrade your license.")
				return
			}
		}
