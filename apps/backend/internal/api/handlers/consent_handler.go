...
func (h *ConsentHandler) RegisterRoutes(r *mux.Router, dbConn *gorm.DB) {
	...
	api.Handle("/user/consents", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.GetUserConsentInTenant))).Methods("GET")
	api.Handle("/user/consentstenantlink",
		middleware.JWTDataPrincipalAuthMiddleware(publicKey)(
	...
	api.Handle("/tenant/{tenantID}/consents", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.GetConsents))).Methods("GET")
	api.Handle("/user/consents/{consentID}/history", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.GetConsentHistory))).Methods("GET")
	api.Handle("/user/consents", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.UpdateConsents))).Methods("PUT")
	api.Handle("/public/user/consents", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.APIGetUserConsentInTenant))).Methods("GET")
	// api.Handle("/user/consents/withdraw", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.WithdrawAllConsents))).Methods("POST")
	api.Handle("/user/consents/withdraw/purpose", middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.WithdrawConsentByPurpose))).Methods("POST")
	api.Handle("/tenant/{tenantID}/logs",
		middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.GetConsentLogs)),
	).Methods("GET")
	api.Handle("/admin/override", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.AdminOverrideConsent))).Methods("POST")
	api.Handle("/admin/tenant/{tenantID}/consents",
		middleware.JWTFiduciaryAuthMiddleware(publicKey)(
			http.HandlerFunc(h.AdminListTenantConsents),
		),
	).Methods("GET")

	api.Handle("/admin/tenant/{tenantID}/users", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.AdminListTenantUsers))).Methods("GET")

	// In your route setup:
	// r.Handle("/api/v1/public/consents",
	// 	middleware.APIKeyAuthMiddleware(dbConn)(
	...
	api.Handle("/guardian/consents/approve",
		middleware.JWTDataPrincipalAuthMiddleware(publicKey)(http.HandlerFunc(h.GuardianApproveConsent)),
	).Methods("POST")
	...
	api.Handle("/admin/consent-links", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.ListConsentLinks))).Methods("GET")
	api.Handle("/admin/consent-links", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.CreateConsentLink))).Methods("POST")
	api.Handle("/admin/consent-links/{linkID}", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.GetConsentLink))).Methods("GET")
	api.Handle("/admin/consent-links/{linkID}", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.UpdateConsentLink))).Methods("PUT")
	api.Handle("/admin/consent-links/{linkID}", middleware.JWTFiduciaryAuthMiddleware(publicKey)(http.HandlerFunc(h.DeleteConsentLink))).Methods("DELETE")
}
