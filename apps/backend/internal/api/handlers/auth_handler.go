...
func FiduciaryLoginHandler(db *gorm.DB, cfg config.Config, privateKey *rsa.PrivateKey) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Check if account is verified (for email/password signups)
		if fiduciary.AuthProvider == "email" && !fiduciary.IsVerified {
			log.Logger.Warn().Str("email", fiduciary.Email).Msg("Login blocked: Account not verified")
			writeError(w, http.StatusForbidden, "Email not verified. Please check your inbox for the verification link.")
			return
		}
...
func ImpersonateUserHandler(db *gorm.DB, auditService *services.AuditService, privateKey *rsa.PrivateKey, ttl time.Duration) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		impersonatorClaims := middleware.GetFiduciaryAuthClaims(r.Context())
...
func UserMeHandler(db *gorm.DB, auditService *services.AuditService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := db.Where("id = ? AND tenant_id = ?", claims.ID, claims.TenantID).First(&user).Error; err != nil {
...
func UpdateUserHandler(db *gorm.DB, auditService *services.AuditService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.GuardianEmail != "" {
			user.GuardianEmail = req.GuardianEmail
		}
...
func GetFiduciaryData(db *gorm.DB, ctx context.Context) (*models.FiduciaryUser, error) {
	claims := middleware.GetFiduciaryAuthClaims(ctx)
	if claims == nil {
